...
public class LogoutSpots extends Module {
    ...
    // track players by entity presence instead of player list
    ...
    public LogoutSpots() {
        super(AddonTemplate.CATEGORY, "logout-spots", "Displays a box where another player has logged out at.");
        lineColor.onChanged();
    }
    ...
    private void onTick(TickEvent.Post event) {
        // Build current player list from world
        List<PlayerEntity> currentPlayers = new ArrayList<>();
        for (Entity entity : mc.world.getEntities()) {
            if (entity instanceof PlayerEntity) currentPlayers.add((PlayerEntity) entity);
        }

        // Any player present in lastPlayers but missing in currentPlayers -> treat as logout/removed
        for (PlayerEntity oldPlayer : lastPlayers) {
            boolean stillPresent = currentPlayers.stream().anyMatch(p -> p.getUuid().equals(oldPlayer.getUuid()));
            if (!stillPresent) {
                if (mc.player != null && oldPlayer.getUuid().equals(mc.player.getUuid())) continue;

                boolean alreadyRecorded = players.stream().anyMatch(e -> e.uuid.equals(oldPlayer.getUuid()));
                if (!alreadyRecorded) add(new Entry(oldPlayer));
            }
        }
